...
static void i2s_parallel_interface_init(uint32_t clk_div)
{
    ...
    esp_intr_alloc(32, 0, _i2s_isr, NULL, NULL);
...
static inline void i2s_parallel_dma_write(uint8_t *buf, size_t length)
{
    int len = length;
    int trans_len = 0;
    int cnt = 0;
    ...
    __dma[cnt-1].eof = 1;
    __dma[cnt-1].empty = (uint32_t)NULL;
...
void i2s_parallel_write_data(uint8_t *data, size_t len)
{
    ...
    int x = 0;
    for (x = 0; x < len / I2S_PARALLEL_BURST_BUFFER_SIZE; x++) {
